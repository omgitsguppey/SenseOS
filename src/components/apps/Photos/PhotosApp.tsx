    ...
    <div className="flex flex-col h-full bg-black text-white relative font-sans overflow-hidden">
      ...
      <div className="absolute top-0 left-0 right-0 h-16 flex items-center justify-between px-4 z-50 bg-gradient-to-b from-black/60 to-transparent pointer-events-none">
        ...
        <div className="flex items-center space-x-3 pointer-events-auto ml-2 flex-shrink-0">
          {user && (
            <label className="bg-zinc-800/80 p-2.5 rounded-full cursor-pointer hover:bg-zinc-700/80 transition-colors backdrop-blur-xl active:scale-95 text-blue-400 shadow-sm focus-within:ring-2 focus-within:ring-blue-500/50">
              <Plus className="w-5 h-5" strokeWidth={2.5}/>
              <input type="file" accept="image/*,video/*" multiple className="sr-only" aria-label="Upload media" ref={fileInputRef} onChange={handleFileUpload} />
            </label>
          )}
          <button 
