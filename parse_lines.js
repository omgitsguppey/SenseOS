...
const collectionsEnd = lines.findIndex((l, i) => i > collectionsStart && l.includes('</div>') && lines[i-1].includes('</div>') && lines[i-2].includes('</div>')); // A bit brittle, let's just grep the end
